function obterListaClubes(){
    return JSON.parse((localStorage.getItem('clube'))) || []; //JSON é para entrar no processo de conversão
}
function salvarListaClubes(lista){
    localStorage.setItem('clubes' , JSON.stringify(lista));

}
let listaClubes = obterListaClubes();

function Adicionar(){
    const clube = document.getElementById('nome-clube').value;
    const tabela = parseFloat(document.getElementById('clube-tabela').value);
    if(clube && tabela){
        listaProdutos.push({clube, tabela});
        salvarListaClubes(listaClubes);
        limparCampos();
    }
}
